...
def main_worker(local_rank,world_size,rank_offset,config):
    ...
    dist.init_process_group(backend='gloo', init_method='file://%s/.pytorch_distribute'%os.getcwd(),world_size=world_size, rank=rank)
    if rank==0:
        try:
    ...
    # train_dataset = ForgeDataset(phase='train', **config.train_dataset)
    # validate_dataset=ForgeDataset(phase='val',**config.val_dataset)
    data_root_path = os.path.abspath('../data')
    train_dataset = ForgeDataset(data_root_path, phase='phase1',usage='train')
    validate_dataset = ForgeDataset(data_root_path, phase='phase1',usage='val')
    ...
    train_loader=torch.utils.data.DataLoader(train_dataset, batch_size=config.batch_size,sampler=train_sampler,
                                             collate_fn=train_dataset.collate_fn,pin_memory=True,num_workers=config.workers)
    validate_loader=torch.utils.data.DataLoader(validate_dataset, batch_size=config.batch_size,sampler=validate_sampler,
                                                collate_fn=validate_dataset.collate_fn,pin_memory=True,num_workers=config.workers)
    logs = {}
    start_epoch = 0
...
def run(logs,data_loader,net,optimizer,local_rank,config,phase='train'):
    ...
    for i, (X, y,_) in enumerate(data_loader):
        ...
        with torch.no_grad():
            ...
            loss_pack['acc'],precision,recall=ACC(logit,y)
        for i in record_list:
...
if __name__=="__main__":
    ...
    for feature in range(2,5):
        ...
        # Config=train_config(name,['ff-all-c23','efficientnet-b4'],attention_layer='',feature_layer=feature_layer,epochs=1,batch_size=2,augment='augment1')

        Config=train_config(name,['efficientnet-b4'],attention_layer='',feature_layer=feature_layer,epochs=1,batch_size=2,augment='augment1')
        Config.mkdirs()
